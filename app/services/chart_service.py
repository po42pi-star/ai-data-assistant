...
class ChartService:
    ...
    def generate_chart(
        ...
        if chart_type not in {"line", "bar", "histogram", "pie", "scatter"}:
            raise FileReadError("Поддерживаются только графики line, bar, histogram, pie и scatter.")
    ...
    def _generate_table_chart(
        ...
        if chart_type == "histogram":
            ...
            if not numeric_columns:
                raise FileReadError("Для pie нужен хотя бы один числовой столбец.")
            ...
            colors = plt.cm.Set3(np.linspace(0, 1, min(len(grouped), 12)))
            axis.pie(grouped.values, labels=grouped.index.astype(str), colors=colors, autopct="%1.1f%%")
            axis.set_title(f"Pie chart: {group_y} by {group_x}")
            description = f"Доля значений \"{group_y}\" по категориям \"{group_x}\"."
        elif chart_type == "scatter":
            if not numeric_columns or len(numeric_columns) < 2:
                raise FileReadError("Для scatter нужны хотя бы два числовых столбца.")
            scatter_x = x_column or numeric_columns[0]
            scatter_y = y_column or numeric_columns[1]
            plot_frame = dataframe[[scatter_x, scatter_y]].copy()
            plot_frame[scatter_x] = pd.to_numeric(plot_frame[scatter_x], errors="coerce")
            plot_frame[scatter_y] = pd.to_numeric(plot_frame[scatter_y], errors="coerce")
            plot_frame = plot_frame.dropna(subset=[scatter_x, scatter_y]).head(200)
            if plot_frame.empty:
                raise FileReadError("Недостаточно данных для scatter графика.")
            axis.scatter(
                plot_frame[scatter_x],
                plot_frame[scatter_y],
                color="#6c8b6b",
                alpha=0.6,
                s=50,
            )
            axis.set_title(f"Scatter plot: {scatter_y} vs {scatter_x}")
            axis.set_xlabel(scatter_x)
            axis.set_ylabel(scatter_y)
            description = f"Зависимость \"{scatter_y}\" от \"{scatter_x}\"."
        else:
            if numeric_columns:
                group_x = x_column or (dimension_columns or list(dataframe.columns))[0]
                group_y = y_column or numeric_columns[0]
    ...
    def _generate_image_chart(self, stored_file: StoredFile, chart_type: str) -> dict[str, Any]:
        ...
        if chart_type == "histogram":
            ...
            axis.pie(values, labels=labels, colors=colors, autopct="%1.1f%%")
            axis.set_title("Mean channel values distribution")
            description = "Распределение средних значений по каналам изображения."
        elif chart_type == "scatter":
            if array.ndim == 3 and array.shape[2] >= 2:
                red = array[:, :, 0].ravel()
                green = array[:, :, 1].ravel()
                sample_indices = np.random.choice(len(red), min(5000, len(red)), replace=False)
                axis.scatter(
                    red[sample_indices],
                    green[sample_indices],
                    color="#d06b4e",
                    alpha=0.4,
                    s=10,
                )
                axis.set_title("Red vs Green channel scatter")
                axis.set_xlabel("Red channel")
                axis.set_ylabel("Green channel")
            else:
                x_profile = grayscale.mean(axis=1).ravel()
                y_profile = grayscale.mean(axis=0).ravel()
                min_len = min(len(x_profile), len(y_profile))
                axis.scatter(
                    x_profile[:min_len],
                    y_profile[:min_len],
                    color="#114b5f",
                ...
                axis.set_title("Vertical vs Horizontal brightness profile")
                axis.set_xlabel("Vertical profile")
                axis.set_ylabel("Horizontal profile")
            description = "Взаимосвязь значений яркости по осям изображения."
        else:
